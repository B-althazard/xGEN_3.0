...
| resolved | Bridge hidden-tab feedback | Added explicit Venice visibility waiting state instead of silently failing when the browser backgrounds the tab. | Makes browser throttling limits visible and keeps recovery behavior understandable. |
...
| resolved | Preset previews | Saved Doll presets now keep the active generated image as their card preview when available. | Makes saved Dolls visually recognizable from the home screen. |
